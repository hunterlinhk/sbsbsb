...
// ============ Font registry ============
// 站点当前使用的字体（取自 src/styles.css / Tailwind 默认体系）
export const BUILTIN_FONTS: { label: string; value: string }[] = [
  { label: "默认（继承站点字体）", value: "" },
  { label: "Inter (站点正文)", value: "Inter, system-ui, -apple-system, sans-serif" },
...
type FontStyleProps = {
  headingFont?: string;
  bodyFont?: string;
  titleBold?: "normal" | "bold" | "black";
  titleItalic?: "normal" | "italic";
};

function headingStyle(p: FontStyleProps): React.CSSProperties {
  return {
    fontFamily: p.headingFont || undefined,
    fontWeight: p.titleBold === "black" ? 900 : p.titleBold === "normal" ? 500 : 700,
    fontStyle: p.titleItalic === "italic" ? "italic" : "normal",
  };
}

type HeroProps = FontStyleProps & {
  eyebrow: string;
  titleLine1: string;
  titleLine2: string;
  intro: string;
  ...
  return (
    <section className="relative overflow-hidden bg-navy-deep text-white" style={{ minHeight: 560, fontFamily: p.bodyFont || undefined }}>
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col px-6 py-24 lg:py-32">
        ...
        <h1 className="text-5xl leading-tight md:text-6xl" style={headingStyle(p)}>
          {p.titleLine1}
          <br />
...
type StatsProps = FontStyleProps & {
  title: string;
    ...
    <section className="bg-navy-deep py-24 text-white lg:py-32" style={{ fontFamily: p.bodyFont || undefined }}>
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-12 text-center">
          ...
          <h2 className="mt-4 text-3xl md:text-4xl" style={headingStyle(p)}>{p.title}</h2>
        </div>
        <div className="grid grid-cols-2 gap-px bg-white/10 lg:grid-cols-4">
          {stats.map((s, i) => (
            <div key={i} className="flex flex-col gap-4 bg-navy-deep p-8 lg:p-10">
              <div className="text-4xl text-white md:text-5xl" style={headingStyle(p)}>{s.v}</div>
...
type CapProps = FontStyleProps & { eyebrow: string; title: string; description: string };
  ...
  return (
    <section className="bg-white py-24 lg:py-32" style={{ fontFamily: p.bodyFont || undefined }}>
      <div className="mx-auto max-w-7xl px-6">
        ...
        <h2 className="mt-4 text-4xl leading-tight text-navy-deep md:text-5xl" style={headingStyle(p)}>{p.title}</h2>
        ...
        <div className="mt-12 grid grid-cols-1 gap-px bg-silver/40 md:grid-cols-2 lg:grid-cols-4">
          {cards.map((c, i) => (
            <div key={i} className="bg-white p-8">
              ...
              <h3 className="mt-6 text-xl text-navy-deep" style={headingStyle(p)}>{c}</h3>
...
type ClientsProps = FontStyleProps & { eyebrow: string; title: string; brands: string };
  ...
  return (
    <section className="bg-silver/40 py-20" style={{ fontFamily: p.bodyFont || undefined }}>
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-10">
          <div className="text-xs uppercase tracking-[0.3em] text-mid-blue">{p.eyebrow}</div>
          <h2 className="mt-3 text-2xl text-navy-deep md:text-3xl" style={headingStyle(p)}>{p.title}</h2>
        </div>
        <div className="grid grid-cols-2 gap-px bg-silver/60 md:grid-cols-3 lg:grid-cols-6">
          {list.map((b, i) => (
            <div key={i} className="flex h-24 items-center justify-center bg-silver/40 text-lg tracking-widest text-navy/60" style={headingStyle(p)}>
              {b}
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-2">
          {cards.map((c, i) => (
            <div key={i} className="overflow-hidden bg-silver/30">
              ...
              <div className="p-8">
                <h3 className="text-2xl text-navy-deep" style={headingStyle(p)}>{c.t}</h3>
...
type CtaProps = FontStyleProps & { eyebrow: string; title: string; description: string; buttonText: string; buttonLink: string };
  ...
  return (
    <section className="relative overflow-hidden bg-navy-deep py-24 text-white lg:py-32" style={{ fontFamily: p.bodyFont || undefined }}>
      ...
      <div className="relative mx-auto max-w-4xl px-6 text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-mid-blue">{p.eyebrow}</div>
        <h2 className="mt-4 text-4xl leading-tight md:text-5xl" style={headingStyle(p)}>{p.title}</h2>
...
// ============ Config builder (so font dropdowns reflect uploaded fonts) ============

type FontOption = { label: string; value: string };

function makeFontFields(extraFonts: FontOption[]): Fields {
  const options = [
    ...BUILTIN_FONTS,
    ...extraFonts.map((f) => ({ label: `${f.label}（自定义）`, value: f.value })),
  ];
  return {
    headingFont: { type: "select", label: "标题字体", options },
    bodyFont: { type: "select", label: "正文字体", options },
    titleBold: {
      type: "select",
      label: "标题粗细",
      options: [
        { label: "常规 (Regular)", value: "normal" },
        { label: "加粗 (Bold)", value: "bold" },
        { label: "特粗 (Black)", value: "black" },
      ],
    },
    titleItalic: {
      type: "radio",
...
export function buildPuckConfig(extraFonts: FontOption[] = []): Config {
  const fontFields = makeFontFields(extraFonts);
  const fontDefaults = { headingFont: "", bodyFont: "", titleBold: "bold", titleItalic: "normal" };
    ...
    components: {
      HeroSection: {
        ...
        fields: {
          ...
          image: { type: "text", label: "背景图片 URL（点击区块后在此处更换）" },
...
export const puckConfig: Config = buildPuckConfig();
